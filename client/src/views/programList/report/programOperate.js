...
const headerList =  ["산림교육", "예방교육", "산림치유", "에너제틱", "릭렉싱", "아트", "쿠킹", "이벤트"];
...
const ProgramOperate = ()=>{
    ...
    const programList = React.useMemo(()=>{
        ...
        const [programName, type, teacherName, inTeacher, outTeacher] = values.slice(i * 5, i * 5 + 5);
    ...
    return <>
        ...
        <h3 className="tableTitle">프로그램운영</h3>
            <Table className="report custom-table">
                <TableHead>
                    <TableRow>
                        <TableCell className="table-header" align="center">서비스유형</TableCell>
                        <TableCell className="table-header" align="center">구분</TableCell>
                        <TableCell className="table-header" align="center">산림교육</TableCell>
                        <TableCell className="table-header" align="center">예방교육</TableCell>
                        <TableCell className="table-header" align="center">산림치유</TableCell>
                        <TableCell className="table-header" align="center">에너제틱</TableCell>
                        <TableCell className="table-header" align="center">릭렉싱</TableCell>
                        <TableCell className="table-header" align="center">아트</TableCell>
                        <TableCell className="table-header" align="center">쿠킹</TableCell>
                        <TableCell className="table-header" align="center">이벤트</TableCell>
                ...
                <TableBody>
                    ...
                    <TableRow>
                        ...
                        {headerList.map((i,key)=> 
                            <TableCell key={key}>{programList.find(item=> item.type === i).programCnt || 0}</TableCell>
                        )}
                    ...
                    <TableRow>
                        ...
                        {headerList.map((i,key)=> 
                            <TableCell key={key}>{programList.find(item=> item.type === i).inTeacher || 0}</TableCell>    
                        )}
                    ...
                    <TableRow>
                        ...
                        {headerList.map((i,key)=> 
                            <TableCell key={key}>{programList.find(item=> item.type === i).outTeacher || 0}</TableCell>
                        )}
