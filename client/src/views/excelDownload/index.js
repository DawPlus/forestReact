...
const ExcelDownload = () => {
  const onClick = () => {
    callApi('/excelData/programList', { openday, endday }).then(
      ({
        data: {
          sheet1,
          sheet2,
        ...
        for (const item1 of sheet1Data) {
          const matchingData2 = sheet3.find(
            (item2) => item1.openday === item2.OPENDAY && item1.agency === item2.agency,
          );
          ...
          if (matchingData2) {
            ...
            newItem.programs = newItem.programs.map((program) => {
              const matchingProgram2 =
                matchingData2.PROGRAM_NAME === program.PROGRAM_NAME &&
        ...
        // 운영현황
        // 운영현황 탭
        const sheet2Result = sheet2
          .map(({ OPENDAY, ...rest }) => ({ ...rest }))
          .map((obj) => Object.values(obj));
        const _sheet2Header = sheet2Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const _sheet2Data = sheet2Result.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        ...
        const _sheet3Header = sheet3Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );

        const _sheet3Data = sheet3Result.map((values) =>
          values.map((value) => ({ v: value || '', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        const sh3 = XLSX.utils.aoa_to_sheet([..._sheet3Header, ..._sheet3Data]);
        sh3['!cols'] = [
        ...
        const _sheet3Header_2 = sheet3Header_2.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const sheet3Result_2 = removeOpenday_2.map((obj) => Object.values(obj));
        const _sheet3Data_2 = sheet3Result_2.map((values) =>
          values.map((value) => ({ v: value || '', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        const sh3_2 = XLSX.utils.aoa_to_sheet([..._sheet3Header_2, ..._sheet3Data_2]);
        sh3_2['!cols'] = [
          { wch: 8 },
          { wch: 8 },
          { wch: 8 },
        ...
        const sheet4Merge = result.map((item, idx) => {
          const matchingItem = sheet4.find(
            (data) => data.TEACHER === item.TEACHER && data.PROGRAM_NAME === item.PROGRAM_NAME,
          );
          return {
            idx: idx + 1,
        ...
        const _sheet4Header = sheet4Header2.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        ...
        const _sheet4Data = sheet4Data.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        ...
        const _sheet5Header = sheet5Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        ...
        const _sheet5Data = sheet5Result.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        ...
        const _sheet6Header = sheet6Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const sheet6Result = sheet6.map((obj) => Object.values(obj));
        const _sheet6Data = sheet6Result.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        ...
        const _sheet7Header = sheet7Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const sheet7Result = sheet7.map((obj) => Object.values(obj));
        const _sheet7Data = sheet7Result.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        ...
        const _sheet8Header = sheet8Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const sheet8Result = sheet8.map((obj) => Object.values(obj));
        const _sheet8Data = sheet8Result.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        ...
        const _sheet9Header = sheet9Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const sheet9Result = sheet9.map((obj) => Object.values(obj));
        const _sheet9Data = sheet9Result.map((values) =>
          values.map((value) => ({ v: value || '0', t: 's', s: defaultStyle })),
        );
        // Create worksheet
        ...
        // 강사현황횟수추가
        // 강사현황횟수추가 탭
        const sheet10Result = sheet10
          .map((i, index) => ({ index: index + 1, ...i }))
          .map((obj) => Object.values(obj));
        const _sheet10Header = sheet10Header.map((item) =>
          item.map((i) => ({ v: i, t: 's', s: headerStyle })),
        );
        const _sheet10Data = sheet10Result.map((values) =>
          values.map((value) => ({ v: value || '', t: 's', s: defaultStyle })),
        );
        // Create worksheet
